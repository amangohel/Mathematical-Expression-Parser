...
parse([$<|Rest]) ->                            % starts with a '<'
      {E1,Rest1}     = parse(Rest),            % then an expression
      [$?|Rest2]    = Rest1,                  % then an '?' operator
      {E2,Rest3}     = parse(Rest2),			% then another expression
	  [$:|Rest4]    = Rest3, 					% then an ':' operator
      {E3,Rest5}     = parse(Rest4),			% then another expression
	  [$>|RestFinal] = Rest5,					%starts with a '>'
      {{cexpr,E1,E2,E3},RestFinal};                 
...
eval(Env,{mul,E1,E2}) ->
    eval(Env,E1) * eval(Env,E2);
eval(Env,{cexpr,E1,E2,E3}) ->		
	case eval(Env,E1) of			%Uses E1 in the case
									
	
	0 -> eval(Env,E2);					%If E1 is 0 then go to E2
	_-> eval(Env,E3)					%Else use E3
...
compile({cexpr,E1,E2,E3}) ->
	compile(E1) ++ compile(E2) ++ compile(E3) ++ [{cexpr3}].  %Compiles etc...
...
run([{push, N} | Continue], Env, Stack) ->  %Pushes N onto the top of the stack
    run(Continue, Env, [N | Stack]);
run([{fetch, A} | Continue], Env, Stack) ->
    run(Continue, Env, [lookup(A,Env) | Stack]);
run([{add2} | Continue], Env, [N1,N2|Stack]) ->  %Takes top two elements and adds them together
    run(Continue, Env, [(N1+N2) | Stack]);
run([{mul2} | Continue], Env ,[N1,N2|Stack]) ->  %Takes the top two elements and multiplies them together
    run(Continue, Env, [(N1*N2) | Stack]);
run([{cexpr3} | Continue], Env, [N1,N2,N3|Stack]) ->	
	case N3 of
...
simplify({add,E1,E2}) ->
	N1 = simplify(E1),
	N2 = simplify(E2),
	case (N1 =/= N1) or (N2 =/= N2) of
		true -> simplify({add,N1,N2});
		_	->{add,N1,N2}
		end;
...
simplify({mul,E1,E2}) ->
	...
	N2 = simplify(E2),
	case (N1 =/= N1) or (N2 =/= N2) of
		true -> simplify({mul,N1,N2});
		_	->{mul,N1,N2}
		end;
		 
simplify({cexpr,E1,E2,E3}) ->
	N1 = simplify(E1),
	N2 = simplify(E2),
	N3 = simplify(E3),
	case (N1 =/= N1) or (N2 =/= N2) or (N3 =/= N3) of
		true -> simplify({cexpr,N1,N2,N3});
		_	->{cexpr,N1,N2,N3}
		end;
